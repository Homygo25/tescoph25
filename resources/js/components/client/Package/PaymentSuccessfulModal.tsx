...
export function PaymentSuccessfulModal({
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // ✅ Validation to prevent missing data causing server error
    if (
      !finalValues?.pck?.id ||
      !finalValues.amount ||
      !finalValues.paymentMode
    ) {
      console.error('Missing required fields:', finalValues);
      alert('Something went wrong. Please reselect the package and try again.');
      return;
    }
    ...
    <Dialog open={open} onOpenChange={onOpen}>
      <DialogContent className="sm:max-w-[700px]">
        ...
        <div className="py-10 text-center">
          <p>Payment Method</p>
          <p className="font-medium">
            {finalValues?.paymentMode === 1
              ? 'Account Balance'
              : finalValues?.paymentMode === 2
              ? 'Online Transfer'
              : ''}
          </p>
        </div>
